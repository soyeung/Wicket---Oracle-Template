prompt creating package body app_refdata.pk_list_sql

create or replace package body app_refdata.pk_list_sql
as
    ...
    ,   p_rds_owning_schema in out app_data.v_list_ref_data.rds_owning_schema%type
    ,   p_rds_table_name    in out app_data.v_list_ref_data.rds_table_name%type
    ,   p_rds_alias         in out app_data.v_list_ref_data.rds_alias%type
    ,   p_rds_column_prefix in out app_data.v_list_ref_data.rds_column_prefix%type
    ,   p_rds_id_sequence   in out app_data.v_list_ref_data.rds_id_sequence%type
    )
    is
    begin

        select
               vlrds.rds_owning_schema
             , vlrds.rds_table_name
             , vlrds.rds_alias
             , vlrds.rds_column_prefix
             , vlrds.rds_id_sequence
           ...
           , p_rds_id_sequence
        from
                  app_data.v_list_ref_data               vlrds
             join app_data.v_editable_ref_data_structure vmrds  on vlrds.rds_id = vmrds.rds_id
       where
             vlrds.rds_id = p_rds_id
           ;
    ...
    is
        ...
        v_rds_column_prefix app_data.v_list_ref_data.rds_column_prefix%type;
        v_rds_owning_schema app_data.v_list_ref_data.rds_owning_schema%type;
        v_rds_admin_view    app_data.v_list_ref_data.rds_admin_view%type;
        v_rds_alias         app_data.v_list_ref_data.rds_alias%type;
    ...
    is
        ...
        v_rds_owning_schema app_data.v_list_ref_data.rds_owning_schema%type;
        v_rds_table_name    app_data.v_list_ref_data.rds_table_name%type;
        v_rds_alias         app_data.v_list_ref_data.rds_alias%type;
        v_rds_column_prefix app_data.v_list_ref_data.rds_column_prefix%type;
        v_rds_id_sequence   app_data.v_list_ref_data.rds_id_sequence%type;
        ...
        begin
            ...
            v_insert_sql :=    'insert '
                           ...
                           || '     , ' || v_rds_alias || '.' || v_rds_column_prefix || pk_ref_data_column_names.c_name_suffix
                           || '     , ' || v_rds_alias || '.' || v_rds_column_prefix || pk_ref_data_column_names.c_is_user_visible_suffix
                           || '     , ' || v_rds_alias || '.' || v_rds_column_prefix || pk_ref_data_column_names.c_order_suffix
                           || '     ) '
                           || 'select '
                           ...
                           || ' where '
                           || '       ' || c_in_dataset_alias || '.' || c_in_dataset_col_prefix || pk_ref_data_column_names.c_id_suffix || ' is null '
                            ;
