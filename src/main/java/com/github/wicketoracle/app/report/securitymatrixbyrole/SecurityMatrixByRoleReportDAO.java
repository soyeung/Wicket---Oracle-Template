...
final class SecurityMatrixByRoleReportDAO extends AbstractOracleDAO
{
    private static final Logger LOGGER = LoggerFactory.getLogger( SecurityMatrixByRoleReportDAO.class );
     ...
     * @throws SQLException
     */
    public SecurityMatrixByRoleReportDAO( final String pUsername, final String pPassword ) throws SQLException
    {
    ...
    {
        setRole( RequiredRoles.ROLE_SECURITY_MATRIX_BY_ROLE_REPORT );

        return getKeyValueRefData( "app_report", "pk_role_matrix_by_role_report", "fn_get_list_ref_data" );
    }
    ...
    public DynamicResult getReport( final int pUserId, final int pDbRoleId ) throws SQLException
    {
        final String dbStatement = " begin "
                                 ...
                                 + "   ? := app_report.pk_role_matrix_by_role_report.fn_get_report( p_aur_id => ? , p_dbrl_id => ?); "
                                 + " end;  ";
        ...
        try
        {
            ...
            dbCstmt.setString( 1, "PK_ROLE_MATRIX_BY_ROLE_REPORT" );
        ...
        catch ( SQLException sqle )
        {
            LOGGER.error
            (
                "SQL Exception whilst running security matrix by role report -> {}; error code -> {}; sql state -> {}; aur id -> {}; dbrl id -> {};"
            ,   new Object []
                {
                    sqle.getMessage()
                ,   sqle.getErrorCode()
