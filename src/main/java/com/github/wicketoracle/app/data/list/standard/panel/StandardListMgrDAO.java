...
final class StandardListMgrDAO extends AbstractListMgrDAO
{
    /** Log */
    private static final Logger LOGGER = LoggerFactory.getLogger( StandardListMgrDAO.class );

    /**
     * Constructor
     */
    public StandardListMgrDAO( final String pUsername, final String pPassword ) throws SQLException
    {
    ...
    public List<ListDatum> getData( final int pDataStructureId, final String pDbRoleName ) throws SQLException
    {
        final String dbStatement = " BEGIN "
                                 ...
                                 + "   ? := app_refdata.pk_list_mgr.fn_get_data ( p_rds_id => ? );"
                                 + " END; ";
        ...
        try
        {
            ...
            dbCstmt.setString( 1, "PK_LIST_MGR" );
    ...
    public void updateData( final int pDataStructureId, final String pDbRoleName, final List<ListDatum> pListData ) throws SQLException , NothingToDoException
    {
        final String dbStatement = " BEGIN "
                                 + "   SYS.dbms_application_info.set_module ( module_name => ? , action_name => ? ); "
                                 + "   app_refdata.pk_list_mgr.pr_update( p_rds_id => ?, p_dataset => ?);"
                                 + " END; ";
